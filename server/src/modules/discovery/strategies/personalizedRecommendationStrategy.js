...
class PersonalizedRecommendationStrategy extends FilterRankingStrategy {
  ...
  buildPreferences(signals) {
    ...
    for (const item of signals.wishlist || []) {
      ...
      buckets.set(priceBucket(item.targetPrice), (buckets.get(priceBucket(item.targetPrice)) || 0) + 2);
    }
  ...
  async recommend(input) {
    ...
    const paged = ranked.slice(start, start + limit)
      .map(({ wine, score }) =>
        publicWine(
          wine,
          hasPersonalSignals ? this.reasonFor(wine, preferences) : "Popular starting point for your cellar",
          score
        )
      );
