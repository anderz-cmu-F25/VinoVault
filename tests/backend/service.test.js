...
beforeEach(() => {
  ...
  CellarEntryModel.findById.mockResolvedValue({ ...MOCK_ENTRY, toObject: () => ({ ...MOCK_ENTRY }) });
...
describe("addManualEntry", () => {
  test("throws if wineName is missing", async () => {
    await expect(addManualEntry("user_1", { quantity: 2 }))
      .rejects.toThrow("wineName is required");
  });

  test("throws if quantity is missing", async () => {
    await expect(addManualEntry("user_1", { wineName: "Margaux" }))
...
describe("editEntry", () => {
  test("throws if entry is not found", async () => {
    ...
    await expect(editEntry("user_1", "entry_1", { quantity: 1 }))
...
describe("removeEntry", () => {
  test("throws if entry is not found", async () => {
    repository.findEntryById.mockResolvedValue(null);
    await expect(removeEntry("user_1", "entry_1"))
      .rejects.toThrow("Cellar entry not found");
  });
