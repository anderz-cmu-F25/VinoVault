...
describe("CellarEntryBuilder — setters", () => {
  let builder;
  beforeEach(() => { builder = new CellarEntryBuilder(); });
...
describe("Director.makeFromInventory", () => {
  test("builds full entry from catalog wine with all optional fields", () => {
    ...
    const result = director.makeFromInventory(
      builder, "user_1", "wine_1", "Barolo", "Conterno", "red", "Piedmont",
      { vintage: 2016, quantity: 12, purchaseDate: "2022-03-01", storageLocation: "Rack C", status: "storing", notes: "Gift" }
    );
...
describe("Director.makeManualEntry", () => {
  test("builds entry from manually typed wine info", () => {
    const builder = new CellarEntryBuilder();
    const director = new Director();
    const result = director.makeManualEntry(
      builder, "user_2", "wine_manual", "My House Wine",
      { winery: "Local Winery", type: "white", region: "Napa", vintage: 2021, quantity: 2 }
    );
  ...
  test("builds entry with only wine name and default quantity", () => {
    ...
    const director = new Director();
    const result = director.makeManualEntry(
      builder, "user_2", "wine_manual", "Unknown Wine", { quantity: 1 }
    );
...
describe("Director.editExistingEntry", () => {
  ...
  test("throws if entry not found", async () => {
    ...
    const builder = new CellarEntryBuilder();
    const director = new Director();
    await expect(director.editExistingEntry(builder, "nonexistent", {}))
      .rejects.toThrow("Cellar entry not found");
  });
});
