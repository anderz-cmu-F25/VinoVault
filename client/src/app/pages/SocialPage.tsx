...
const API_BASE = (import.meta.env.VITE_SERVER_URL || 'http://localhost:3000') + '/api/social'
...
function normalizeRelationshipState(raw: any, currentUserId?: string | null): RelationshipState {
  ...
  if (value.includes('PENDING') || value.includes('REQUEST')) {
    if (currentUserId && raw?.requestedBy && String(raw.requestedBy) === String(currentUserId)) {
      return 'REQUEST_SENT'
    }
...
export function SocialPage() {
  ...
  async function authJsonFetch(url: string, options: RequestInit = {}): Promise<any> {
  ...
  function normalizeFriend(item: any): Friend | null {
    ...
    return {
      id: otherUserId,
      name: nestedUser?.username || nestedUser?.name || item.username || item.name || fallbackName,
  ...
  async function loadFriendRequests() {
    try {
      const result = await tryJsonEndpoints([`${API_BASE}/friend-requests`, `${API_BASE}/friends`])
  ...
  async function handleSearchUserByUsername() {
    try {
      ...
      setSearchedUser({
        ...
        relationshipState: normalizeRelationshipState(userResult, currentUserId) || 'NO_FRIENDSHIP',
  ...
  async function handleViewEventDetails(eventId: string) {
    try {
      ...
      setSelectedEvent({
        ...
        location: event.location || 'TBD',
        host:
          event.hostUsername || event.hostName || event.host || event.hostUserId || 'Unknown Host',
        hostUserId: event.hostUserId || event.hostId || event.createdBy || event.userId,
        description: event.details || event.description,
        joined: Boolean(event.joined ?? event.isJoined ?? false),
        isPast,
            ...
            {isLoadingFriends ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                {filteredFriends.map((friend) => {
                  ...
                  const blockedByMe =
                    !!friend.blockedBy && String(friend.blockedBy) === String(currentUserId)
