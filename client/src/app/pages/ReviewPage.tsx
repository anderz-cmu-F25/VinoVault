...
export function ReviewPage() {
  ...
  const fetchWines = async (query: string) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/wines?q=${encodeURIComponent(query)}&limit=50`, {
  ...
  const emptyState = useMemo(() => !loading && wines.length === 0, [loading, wines.length])
  ...
  return (
    <main className="max-w-5xl mx-auto px-8 py-16">
      ...
      {loading ? (
        ...
        <div className="bg-white p-12 text-center" style={{ borderRadius: '16px' }}>
          <h3
