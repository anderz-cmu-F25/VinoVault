...
export function SignupPage() {
  ...
  const completeOAuthSignUp = async (usernameOverride?: string) => {
    ...
    try {
      ...
      setError(clerkError.errors?.[0]?.message ?? 'Google sign-up could not be completed.')
    } finally {
      setIsLoading(false)
    }
  }
              ...
              style={{
                ...
                cursor:
                  isLoading || (isUsernameMissing && !continuationUsername.trim())
                    ...
                    : 'pointer',
                border: 'none',
                opacity: isLoading || (isUsernameMissing && !continuationUsername.trim()) ? 0.7 : 1,
              }}
            >
      ...
      >
        ...
        <form onSubmit={handleSignUp}>
          ...
          {error && (
            <p
              className="mb-4 text-center"
              style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '13px', color: '#DC2626' }}
            >
              {error}
            </p>
          )}
